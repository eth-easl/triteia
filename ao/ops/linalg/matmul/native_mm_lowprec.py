...
def native_matmul_lowprec_248(
    ...
    infeatures = qweight.shape[0] // bitwidth * 32 # qweight is stored in 32-bit integers (packed)
    ...
    assert qzero.shape[1] == outfeatures // 32 * bitwidth, f"qzero has incorrect shape, should be (1, outfeatures// 32 * bitwidth). Expected (1, {infeatures // 32 * bitwidth, outfeatures}), got {qzero.shape}"
    assert scale.shape[1] == outfeatures, f"scale has incorrect shape, should be (1, outfeatures). Expected (1, {outfeatures}), got {scale.shape}"
    
    out_shape = x.shape[:-1] + (outfeatures,)
    wf = torch.tensor(
        list(range(0, 32, bitwidth)), dtype=torch.int32
    ).unsqueeze(0)
    ...
    if num_itr == 1:
        weights = scale[g_idx.long()] * (
            weight - zeros[g_idx.long()]
        )
    else:
