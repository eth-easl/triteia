"""
adapted from:
https://raw.githubusercontent.com/microsoft/BitBLAS/main/python/bitblas/module/__init__.py
...
class Linear(nn.Module):
    ...
    def repack_from_weights(self, qweight, scales, qzeros, bias=None):
        qweight = qweight.T.contiguous().view(self.TORCH_STORAGE_DTYPE)
        if self.bitblas_matmul.weight_transform is not None:
            qweight = self.bitblas_matmul.weight_transform(qweight.cpu()).cuda()
        self.qweight = qweight
        scales = scales.T.contiguous().view(self.torch_dtype)
        self.scales = scales
        # qzeros should be dequantized to int zeros.
        intzeros = unpack_qzeros(qzeros, self.bits).T.contiguous()
        if self.bitblas_matmul.config.zeros_mode == "original":
            self.zeros = intzeros.to(torch.float16).contiguous()
        elif self.bitblas_matmul.config.zeros_mode == "rescale":
            self.zeros[:, :] = intzeros.to(torch.float16)[:, :] * self.scales[:, :]
        ...
        if self.bias is not None:
            if bias is None:
                raise ValueError("Bias is not provided.")
            self.bias = bias.data.to(torch.float16).contiguous()
