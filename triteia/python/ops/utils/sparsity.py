...
def calculate_sparsity(x: torch.Tensor):
    return 1 - (x != 0).sum().float() / x.numel()


def mask_creator(x: torch.Tensor, n: int = 2, m: int = 4):
    """
    Create N:M sparsity masks
    """
    mask = None
    if x.numel() % m != 0:
        raise ValueError(
            f"Tensor of size {x.shape} can't be evenly divided into " f"{m} groups"
        )

    num_groups = x.numel() // m

    # N:M sparsity for linear layers
    tensor_temp = x.detach().abs().reshape(num_groups, m)
    index = torch.argsort(tensor_temp, dim=1)[:, : int(m - n)]

    w_b = torch.ones(tensor_temp.shape, device=tensor_temp.device)
    mask = w_b.scatter_(dim=1, index=index, value=0).reshape(x.shape)
