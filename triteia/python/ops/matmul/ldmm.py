...
def ldmm (indices, x, LwA, LwB, DeltaW, metas, ss, base_weight=None):
  """
  Args:
    indices: Shape: `[B]`. Contains the index of the model for each input vector in the batch B. 
    The indices can be in range [0:M+N-1], where M is the number of lora models and N is the number of sbmm models.
    An index i < M represents the i'th model of the lora models,
    whereas an index i >= M represents the (i-M)'th model of the sbmm models.
    x: Shape: `[B, H1]`. Batch with B input vectors of size H1.
    LwA: Shape: `[M, H1, R]`. M weight matrices of size H1 by R, R being the rank of the lora models
    LwB: Shape: `[M, R, H2]`. M weight matrices of size R by H2, R being the rank of the lora models
    DeltaW: Shape: `[N, H1, H2]`. N qweight matrices of size H1, H2 for the sbmm models
    metas: metadata for sbmm
    ss: ss for sbmm
  """
  ...
  mask_lora = indices < M and indices != -1
  mask_sbmm = indices >= M and indices != -1
