...
if project_root is None:
    print(
        "There is no project root set (env: thunderkittens_root) did you run env.src?"
    )
    os._exit(-1)


def _sources(name):
    return [f"{name}_frontend.cpp", f"{name}.cu"]
...
def jit_build(name, debug=False, gpu_type="4090"):
    ...
    if debug:
        _cuda_flags += ["-D__DEBUG_PRINT", "-g", "-G", "-D TORCH_USE_CUDA_DSA"]
    return load(
        name=f"{name}",
...
def cuda_extension(name, debug, gpu_type):
    ...
    return CUDAExtension(
        f"{name}",
        sources=_sources(name),
        extra_compile_args={"cxx": ["-std=c++20"], "nvcc": ["-O3"] + _cuda_flags},
        libraries=["cuda"],
    )
...
def _rtile(b, n, d, dt):
    return torch.randn(b, n, d, device="cuda", dtype=dt) / (n * d)


def _rhtile(b, h, n, d, dt):
    return torch.randn(b, h, n, d, device="cuda", dtype=dt) / (n * d)


def _rones(b, n, d, dt):
    return torch.ones(b, n, d, device="cuda", dtype=dt)
