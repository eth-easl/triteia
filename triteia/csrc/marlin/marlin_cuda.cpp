...
int marlin_cuda_bmm_2_4(const void *A, const void *B, void *C, void *s, int prob_m,
                int prob_n, int prob_k, void *workspace, int groupsize = -1,
                int dev = 0, cudaStream_t stream = 0, int thread_k = -1,
                int thread_n = -1, int sms = -1, int max_par = 16);
...
void bmm_2_4(const torch::Tensor &A, const torch::Tensor &B, torch::Tensor &C,const torch::Tensor &s, const torch::Tensor &indices, torch::Tensor &workspace,int thread_k = -1, int thread_n = -1, int sms = -1, int max_par = 8) {
  //   A: [r, k], m: # reqs, k: in features
  //   B: [r, n, k], r: # models, n: out features, k: in features
  //   C: [r, n], m: # reqs, n: out features
  ...
  if (workspace.numel() < prob_n / 128 * max_par)
    AT_ERROR("workspace must be of size at least ", prob_n / 128 * max_par,
             ".");
  int dev = A.get_device();
  int err = marlin_cuda_bmm_2_4(A.data_ptr(), B.data_ptr(), C.data_ptr(), s.data_ptr(),
                        prob_r, prob_n, prob_k, workspace.data_ptr(), groupsize,
                        dev, at::cuda::getCurrentCUDAStream(dev), thread_k,
                        thread_n, sms, max_par);
  if (err == ERR_PROB_SHAPE) {
    AT_ERROR("Problem (r=", prob_r, ", n=", prob_n, ", k=", prob_k, ")",
             " not compatible with thread_k=", thread_k,
             ", thread_n=", thread_n, ".");
  } else if (err == ERR_KERN_SHAPE) {
    AT_ERROR("No kernel implementation for thread_k=", thread_k,
